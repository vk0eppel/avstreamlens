...
- **Dante device metadata enrichment (channels, sample rate, bit depth)** — Dante devices advertise device name, channel names/counts, sample rates, and bit depths during discovery (per Audinate docs), but we currently extract only the instance name from mDNS. Where the metadata lives on the wire: (1) **mDNS TXT records** on `_netaudio-arc`/`_netaudio-cmc` announcements — key/value pairs (protocol versions, manufacturer/model, possibly capabilities); multicast, so visible from any port; easiest first step — extend `parser/mdns.rs` to walk the TXT RDATA after the instance name. (2) **ARCP (UDP 4440)** — how Dante Controller reads channel names/routing; unicast request/response, needs SPAN. (3) **ConMon multicast (`224.0.0.230–233`, ports 8700–8708)** — "Multicast Control and Monitoring"; joinable passively, the best lead for SPAN-free device status/sample-rate enrichment, but framing is undocumented. All three are proprietary — capture and reverse-engineer on a live Dante network before writing any parser (see Field Verification). Prize: `📇 Discovered (mDNS)` shows name + model + sample rate instead of name only. (`src/parser/mdns.rs`; `src/capture.rs` — `dante_names` → richer struct; `src/report.rs` — `print_discovery`)

- **Join Dante's SAP group `239.255.255.255`** — Audinate's official port list says Dante announces AES67 sessions via SAP to **`239.255.255.255:9875`**, not the classic `224.2.127.254` we join at startup. On an IGMP-snooping switch we would never receive Dante AES67 SAP announcements — `239.255.255.255` is inside `239.255/16` so it is snooped, not flooded. Low-risk fix: add `239.255.255.255` to the startup join list alongside `224.2.127.254` (SAP listeners should monitor both). (`src/main.rs` — `join_multicast_groups`)
...
- **Dante audio port ranges vs. official Audinate list** — ⚠ DISCREPANCY (2026-06-12). Audinate's official port list (support article 5508285534239, "Which network ports does Dante use?", updated 2026-05-06) does **not mention the 5000–6000 range at all**. It says: multicast ATP audio = `239.255.0.0/16` **port 4321**; unicast audio/video flows = **UDP 14336–15359** (Via: 34336–34600); AES67-mode multicast = `239.69.0.0/16:5004`. Our `is_likely_dante_audio` (src AND dst even in 5000–6000) and `is_dante_multicast` (even dst 5000–6000) are built entirely on the 5000–6000 heuristic. If the article is accurate: multicast ATP on dst 4321 fails our port gate and is missed (though ATP is likely not RTP-framed, so it may fail the RTP gate anyway and be invisible rather than misclassified), and unicast flows in 14336–15359 are likewise missed. Before changing anything, capture a live Dante flow (unicast + multicast) and record the actual ports: the 5000–6000 heuristic may reflect real observed traffic on older firmware. Also check whether ATP (port 4321) packets pass the RTP version-2 gate at all — if not, multicast Dante detection may need a non-RTP ATP path. (`src/parser.rs` — `is_likely_dante_audio`, `is_dante_multicast`, `detect_protocol`)
...
- **Dante mDNS TXT records and ConMon content** — Prerequisite for the metadata-enrichment feature above. (a) **TXT records — still open**: capture the full TXT RDATA of `_netaudio-arc._udp` / `_netaudio-cmc._udp` announcements and record which keys exist (versions? manufacturer? model? sample rate? channel counts?) across at least two vendors. (b) **ConMon — ✅ PARTIALLY VERIFIED (2026-06-12)** from a live Wireshark capture (Audinate Brooklyn-class device `00:1d:c1:19:86:2a` + Yamaha `ac:44:f2:84:1e:60`, both on 169.254.x.x):
  ...
  Next: capture port 8700/8701 traffic and a TXT dump; decide whether TXT alone or TXT+8705-metering is the implementation target. (`src/parser/mdns.rs`; future `src/parser/conmon.rs`)
